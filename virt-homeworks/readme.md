...
Создадим  S3 bucket  в Yandex Cloud, для этого создадим terraform-файл:
```terraform
terraform {
 required_providers {
   yandex = {
...
`Golang` установлен
```bash
nik@ubuntuVM:~/Downloads$ sudo su
root@ubuntuVM:/home/nik/Downloads# rm -rf /usr/local/go && tar -C /usr/local -xzf go1.19.4.linux-amd64.tar.gz
root@ubuntuVM:/home/nik/Downloads# export PATH=$PATH:/usr/local/go/bin
